...
- `src/lib/firestore.ts` — CRUD completo: users, teachers, grades, students, roles + `seedInitialData()`
- `src/contexts/AuthContext.tsx` — Auth con roles dinámicos, `hasPermission()`, `isRole()`
...
- `src/components/admin/` — Panel admin: Dashboard, CRUD, RolesManager
