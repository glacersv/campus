...
export default function RoleLayout({ modules, moduleIcons, moduleColors, children }: RoleLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-[#F3F5F6] overflow-y-auto">
      {/* Sidebar */}
      <aside className={`${collapsed ? 'w-[72px]' : 'w-64'} bg-white border-r border-slate-200 flex flex-col justify-between shrink-0 transition-all duration-300`}>
        ...
        <header className="h-14 bg-white border-b border-slate-200 px-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            <h1 className="text-base font-semibold text-slate-900 font-display">
              {SYSTEM_MODULES.find(m => m.id === currentPath)?.label || roleLabel}
            </h1>
          </div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-semibold text-primary bg-primary-light px-2.5 py-1 rounded-full">{roleLabel}</span>
          </div>
        </header>
