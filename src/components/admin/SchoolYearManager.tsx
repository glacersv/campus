...
export default function SchoolYearManager() {
  ...
  rows.forEach(r => {
    ...
    if (selectedCycle !== 'all' && cy !== selectedCycle) return;
    if (!groupedRows[cy]) {
      groupedRows[cy] = [];
    }
    ...
    <div className="space-y-8 fade-in max-w-7xl mx-auto pb-12">
      ...
      <div className="space-y-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          {/* Interactive Cycle Filter Pill Tabs */}
          <div className="flex items-center gap-1.5 bg-slate-100/80 p-1.5 rounded-xl border border-slate-200/80 overflow-x-auto self-start lg:self-auto">
            <button
              type="button"
