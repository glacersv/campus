...
export default function SchoolYearManager() {
  ...
  const filteredCycles = selectedCycle === 'all'
    ? activeCycles
    : activeCycles.filter(c => c === selectedCycle);
    ...
    <div className="space-y-8 fade-in max-w-7xl mx-auto pb-12">
      ...
      <div className="relative card-crema p-6 md:p-8 overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all hover:border-slate-300">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto relative z-10">
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-gradient-to-r from-primary/5 via-accent/5 to-secondary/10 border border-primary/20 rounded-full text-xs font-bold text-primary shadow-xs">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2 pb-1">
          <span className="text-xs font-bold text-slate-400 mr-1.5 flex items-center gap-1.5">
...
function GradeMigrationCard({
  ...
  const isReconfigured =
    r.currentSectionNames.length !== r.newSectionNames.filter(n => n.trim()).length;
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-2 min-w-0">
          <div className="min-w-0 flex-1">
            <h4
              className="text-sm font-black text-slate-900 font-display group-hover/card:text-primary transition-colors leading-tight truncate"
              title={r.gradeName}
            >
              {r.gradeName}
            </h4>
            <span
              className={`inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-md text-[9px] font-bold border ${
                r.cycle === 'parvularia'
          ...
          <div className="shrink-0">
            {r.nextGradeId === null ? (
              ...
              <span
                className="text-[9px] text-slate-500 font-bold uppercase tracking-wider bg-slate-100 px-2 py-1 rounded-md block max-w-[85px] truncate"
        ...
        {/* Enrollment flow: Matrícula Entrante vs Padrón Saliente */}
        <div className="space-y-1.5 bg-slate-50 border border-slate-100 rounded-xl p-2.5">
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="font-bold text-primary truncate max-w-[85px]" title={r.sourceGradeName}>
              {r.sourceGradeName}
            </span>
          </div>
          <div className="flex justify-between items-center text-xs border-t border-slate-100/60 pt-1.5 mt-1">
            ...
            <span
              className="font-bold text-slate-700 truncate max-w-[85px]"
              title={r.nextGradeName === 'Graduación' ? 'Egreso' : r.nextGradeName}
            >
          ...
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(dist).map(([name, count]) => (
              ...
              >
                ...
                <span className="text-primary font-black font-mono text-[11px]">{count}</span>
              </span>
            ))}
