...
export default function SchoolYearManager() {
  ...
  const visibleCycles = activeCycles.filter(c => selectedCycle === 'all' || c === selectedCycle);
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-xs font-bold text-secondary font-mono card-crema px-4 py-2">
              {selectedCycle === 'all' ? rows.length : rows.filter(r => r.cycle === selectedCycle).length} Grados
            </span>
          </div>
        </div>
