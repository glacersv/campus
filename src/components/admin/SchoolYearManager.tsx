...
import { Student, Cycle, CYCLE_NAMES } from '../../types';
import {
  MigrationPlanRow,
  buildMigrationRows,
  resizeNames,
...
export default function SchoolYearManager() {
  ...
  // Filter rows based on cycle filter
  const filteredRows = rows.filter(
    r => selectedCycle === 'all' || r.cycle === selectedCycle
  );
  ...
  const activeCycles = CYCLE_KEYS.filter(c => groupedRows[c] && groupedRows[c].length > 0);
  Object.keys(groupedRows).forEach(c => {
    if (!activeCycles.includes(c as Cycle) && groupedRows[c].length > 0) {
    ...
    <div className="space-y-8 fade-in max-w-7xl mx-auto pb-12">
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          {/* Cycle Filter Pills Selector */}
          <div className="flex items-center gap-1.5 bg-slate-100/80 p-1.5 rounded-xl border border-slate-200 self-start sm:self-auto flex-wrap">
            <button
              type="button"
        ...
        {filteredRows.length === 0 ? (
          <div className="text-center py-16 card-crema text-tertiary">
            ...
            <p className="text-sm font-medium text-slate-600">
              No se encontraron grados configurados para el filtro de ciclo seleccionado.
            </p>
          </div>
        ) : (
      ...
      {/* Preservation & Activation Footer */}
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        ...
        {/* Enrollment flow - Matrícula Entrante vs Egreso */}
        <div className="space-y-1.5 bg-slate-50 border border-slate-100 rounded-xl p-2.5">
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span
              className="font-bold text-primary truncate max-w-[85px]"
