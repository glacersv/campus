...
import { FileText, Users, AlertCircle, Search, ChevronDown, X, Check, Plus, Crown } from 'lucide-react';
...
export default function FormularioProyecto({ proyectoInicial, onCancel, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    // Skip reset in edit mode — section comes from the project
    if (esEdicion) return;
  ...
  async function handleBorrador() {
    ...
    try {
      ...
      if (esEdicion && proyectoInicial) {
        const rep = integrantesData.find(i => i.es_rep) || integrantesData[0];
        await guardarBorrador(proyectoInicial.id, {
          ...baseData,
          integrantes: integrantesData.map(i => i.uid),
          integrantes_detalle: integrantesData,
          representante_id: rep?.uid || proyectoInicial.representante_id,
          representante_nombre: rep?.nombre || proyectoInicial.representante_nombre,
        });
      } else {
        await crearProyecto({
          ...baseData,
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-4 mb-4">
          <FormField label="Grado *" error={errores.grado}>
            {loadingProfile ? (
              ...
              <div className="flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl border border-slate-200">
                {(esEdicion ? [grado].filter(Boolean) : GRADOS_PROYECTO).map(g => (
                  <button
                    key={g}
                    type="button"
                    onClick={() => { if (userProfile?.role !== 'alumno' && !esEdicion) setGrado(g); }}
                    disabled={userProfile?.role === 'alumno' || esEdicion}
                    className={`px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                      grado === g
                        ? 'bg-primary text-white shadow-xs'
                        : 'text-slate-600 hover:bg-slate-200/50'
                    } ${userProfile?.role === 'alumno' || esEdicion ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
                    {g}
                  </button>
                ))}
          ...
          <FormField label="Sección *" error={errores.seccion}>
            {loadingProfile ? (
              ...
              <div className="flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl border border-slate-200">
                {(esEdicion ? [seccion].filter(Boolean) : secciones).map(s => (
                  <button
                    key={s}
                    type="button"
                    onClick={() => { if (grado && !esEdicion) setSeccion(s); }}
                    disabled={!grado || esEdicion}
                    className={`px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                      seccion === s
                        ? 'bg-primary text-white shadow-xs'
                        : 'text-slate-600 hover:bg-slate-200/50'
                    } ${!grado || esEdicion ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
                    {s}
                  </button>
                ))}
      ...
      <div className="p-5">
        ...
        {!grado || !seccion ? (
          ...
          <>
            <div className="grid grid-cols-[1fr_80px_auto] gap-2 mb-2 text-[10px] text-slate-400 font-medium">
              <span>Nombre completo</span><span className="text-center">N° lista</span><span className="w-7" />
            </div>
                ...
                <div className="grid grid-cols-[1fr_80px_auto] gap-2 items-center">
                  ...
                  {int.uid && (
                    <button
                      type="button"
                      onClick={() => {
                        setIntegrantes(prev => prev.map((it, i) => ({
                          ...it,
                          es_rep: i === idx ? !it.es_rep : false
                        })));
                      }}
                      ...
                      title={int.es_rep ? 'Es el líder' : 'Marcar como líder'}
                    >
