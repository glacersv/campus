...
import {
  ...
  FECHA_LIMITE_REGISTRO, FECHA_LIMITE_APROBACION, MATERIAS
} from '../types'
...
export function useProyectos() {
  ...
  // ── Suscripción en tiempo real según rol ───────────────────
  useEffect(() => {
    if (!perfil) return
    setLoading(true)
    ...
    if (perfil.rol === 'estudiante') {
      // Solo proyectos donde el usuario es integrante
      q = query(col,
        where('integrantes', 'array-contains', perfil.id),
        orderBy('fecha_registro', 'desc')
      )
    } else if (perfil.rol === 'docente') {
      // Proyectos en estados de revisión de materia
      q = query(col,
        where('estado', 'in', ['registrado', 'en_revision_materia']),
        orderBy('fecha_registro', 'desc')
      )
    } else {
      // Coordinación y admin ven todos
  ...
  async function crearProyecto(data: CrearProyectoData): Promise<{ id?: string; error?: string }> {
    ...
    // Verificar título duplicado en mismo grado+sección
    ...
    try {
      const ref = await addDoc(collection(db, 'proyectos'), {
        ...
        representante_nombre: perfil.nombre,
  ...
  // ── ESTUDIANTE: enviar a validación ───────────────────────
  ...
  // ── COORDINACIÓN: aprobar oficialmente ────────────────────
  ...
  // ── COORDINACIÓN: rechazar oficialmente ───────────────────
